...
@implementation SFApi_MessageNotification

/*
 * 反序列化函数，用于从json字符串反序列化本类型实例
 */
+ (SFApi_MessageNotification *) deserializeWithJsonData:(NSData *) jsonData
{
    ...
    if (error)
    {
        @throw [[SFLocalException alloc] initWithCode:LocalExceptionCode_DESERIALIZE_ERR message:@"SFApi_MessageNotification 反序列化失败"];
    }
    return [SFApi_MessageNotification deserialize:jsonDict];
}

/*
 * 反序列化函数，用于从json节点对象反序列化本类型实例
 */
+ (SFApi_MessageNotification *) deserialize:(NSDictionary *)json
{
      if (!([json isKindOfClass:[NSNull class]] || json == nil)) {
          SFApi_MessageNotification *result = [[SFApi_MessageNotification alloc] init];
