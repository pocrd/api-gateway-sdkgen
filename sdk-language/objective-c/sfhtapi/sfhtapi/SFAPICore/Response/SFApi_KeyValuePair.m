...
@implementation SFApi_KeyValuePair

/*
 * 反序列化函数，用于从json字符串反序列化本类型实例
 */
+ (SFApi_KeyValuePair *) deserializeWithJsonData:(NSData *) jsonData
{
    ...
    if (error)
    {
        @throw [[SFLocalException alloc] initWithCode:LocalExceptionCode_DESERIALIZE_ERR message:@"SFApi_KeyValuePair 反序列化失败"];
    }
    return [SFApi_KeyValuePair deserialize:jsonDict];
}

/*
 * 反序列化函数，用于从json节点对象反序列化本类型实例
 */
+ (SFApi_KeyValuePair *) deserialize:(NSDictionary *)json
{
      if (!([json isKindOfClass:[NSNull class]] || json == nil)) {
          SFApi_KeyValuePair *result = [[SFApi_KeyValuePair alloc] init];
