...
@implementation SFApi_StringArrayResp
- (id) init
{
    if (self = [super init])
    {
...
+ (SFApi_StringArrayResp *) deserializeWithJsonData:(NSData *) jsonData
{
    ...
    if (error)
    {
        @throw [[SFLocalException alloc] initWithCode:LocalExceptionCode_DESERIALIZE_ERR message:@"SFApi_StringArrayResp 反序列化失败"];
    }
    return [SFApi_StringArrayResp deserialize:jsonDict];
}
...
+ (SFApi_StringArrayResp *) deserialize:(NSDictionary *)json
{
      if (!([json isKindOfClass:[NSNull class]] || json == nil)) {
          SFApi_StringArrayResp *result = [[SFApi_StringArrayResp alloc] init];
