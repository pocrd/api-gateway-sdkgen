...
@implementation SFApi_KeyValueList
- (id) init
{
    if (self = [super init])
    {
...
+ (SFApi_KeyValueList *) deserializeWithJsonData:(NSData *) jsonData
{
    ...
    if (error)
    {
        @throw [[SFLocalException alloc] initWithCode:LocalExceptionCode_DESERIALIZE_ERR message:@"SFApi_KeyValueList 反序列化失败"];
    }
    return [SFApi_KeyValueList deserialize:jsonDict];
}
...
+ (SFApi_KeyValueList *) deserialize:(NSDictionary *)json
{
      if (!([json isKindOfClass:[NSNull class]] || json == nil)) {
          SFApi_KeyValueList *result = [[SFApi_KeyValueList alloc] init];
        ...
        if (keyValueArray && ![keyValueArray isKindOfClass:[NSNull class]]) {
            ...
            NSDictionary *dict = [keyValueArray objectAtIndex:i];
               if (dict && ![dict isKindOfClass:[NSNull class]]) {
                   [result.keyValue addObject:[SFApi_KeyValuePair deserialize:dict]];
               }
            }
        }
...
- (NSMutableDictionary *) serialize
{
    ...
    if (self.keyValue && ![self.keyValue isKindOfClass:[NSNull class]]) {
        ...
        for (NSInteger i = 0; i < [self.keyValue count]; i++) {
            SFApi_KeyValuePair *value = self.keyValue[i];
