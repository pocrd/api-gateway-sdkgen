...
//#import "SFAppChannelManager.h"

#define kAccessGroup @""

#import "SFApiCode.h"

@interface SFApiContext ()

@property (nonatomic, strong) NSString *deviceToken;
@end


@implementation SFApiContext
...
- (NSString *) parameterStringWithRequest:(SFBaseRequest *)request
{
    return [self parameterStringWithRequests:[NSArray arrayWithObject:request]];
}

- (SFServerResponse *) fillResponseWithRequests:(NSArray *)requests data:(NSData *)data
{
    if (data)
    {
        @try {
            ...
            if (error)
            {
                for (SFBaseRequest *request in requests) {
...
- (SFServerResponse *) fillResponseWithRequest:(SFBaseRequest *)request data:(NSData *)data
{
    return [self fillResponseWithRequests:[NSArray arrayWithObject:request] data:data];
}

- (SFServerResponse *) fillResponseWithRequests:(NSArray *)requests dictionary:(NSDictionary *)jsonDict
{
    SFServerResponse *serverResp = nil;
    if (jsonDict)
    {
        NSArray *content = [jsonDict objectForKey:@"content"];
        SFApi_Response *resp = [SFApi_Response deserialize:[jsonDict objectForKey:@"stat"]];
        if (resp)
        {
            serverResp = [[SFServerResponse alloc] initWithResponse:resp];
            if (resp.code == SFApiCode_SUCCESS)
            {
                if (resp.stateList && resp.stateList.count == requests.count)
                {
                    ...
                    {
                        SFApi_CallState *state = [stateList objectAtIndex:i];
                        SFBaseRequest *request = [requests objectAtIndex:i];
...
- (SFServerResponse *) fillResponseWithRequest:(SFBaseRequest *)request dictionary:(NSDictionary *)jsonDict
{
...
- (NSString *) parameterStringInternalWithDictionary:(NSMutableDictionary *)params SecurityType:(NSInteger)securityType
{
    ...
    {
        @throw [[SFLocalException alloc] initWithCode:LocalExceptionCode_TOKEN_MISSING];
    }
...
- (void) fillErrorWithRequest:(SFBaseRequest *)request code:(NSInteger)code
{
...
- (void) fileErrorWithRequests:(NSArray *)requests code:(NSInteger)code
{
    ...
    {
        SFBaseRequest *request = requests[i];
        [request fillResponse:code length:0 message:@"" input:nil];
    }
}
